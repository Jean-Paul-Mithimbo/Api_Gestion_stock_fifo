...
class Article(models.Model):
    ...
    seuil_areaprovisionnement=models.PositiveIntegerField(default=10)
...
class StockEntry(models.Model):
    ...
    def __str__(self):
        return f"{self.article.nom}-{self.quantite} unité entrées le{self.date_entree}"
...
class StockMouvement(models.Model):
    ...
    def __str__(self):
        return f"{self.article.nom}-{self.quantite} unités sortie le{self.date_sortie}"